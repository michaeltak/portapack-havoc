/***********************************************************************
 * $Id:: mw_usbd_core.h 2221 2015-12-17 23:28:25Z usb00423                     $
 *
...
typedef struct _USB_CORE_DESCS_T {
	...
	uint8_t *full_speed_desc;	/**< Pointer to USB device configuration descriptor
								 ...
								 * For High speed capable device this descriptor must
								 * be at writeable memory (ex. RAM) and should not be in
								 * Read-Only memory (ex. FLASH).
								 */
	...
	uint8_t *device_qualifier;	/**< Pointer to USB device qualifier descriptor. For
								 ...
								 * be set to null (0).
								 */
	uint8_t *bos_descriptor;	/**< Pointer to USB BOS descriptor. For LPM Mode only.
								 */
} USB_CORE_DESCS_T;
...
typedef struct USBD_API_INIT_PARAM {
	...
	uint8_t max_num_ep;	/**< max number of endpoints supported by the USB device
						 ...
						 */
	uint8_t high_speed_capable;	/**< Specifies USB device controller's speed capability.
						0 : Full-speed only; 1 : High speed capable */
	uint8_t double_buffer; /**< Specifies whether low-level HW driver to use double buffering for EP transfers.*/
	uint8_t pad0[1];
	...
	uint32_t lpm_setting;		/**< Configurable LPM setting */

	/** Call back function to handle get string descriptor request. 
	 *
	 *  This callback is invoked by stack when get string descriptor request is received. If callback returns with pD pointing to NULL,
	 *  then the stack uses \em string_desc descriptor array provided during initialization to find the string. This interface
	 *  provides mechanism to override the stack handler for string desriptor and helps in handling dis-contiguous string index descriptor requests. 
	 *	For example Windows host request string descriptor at index 0xEE to see if the devie supports Microsoft defined OS descriptors.
	 ...
	 *  \param[out] pD  Double Pointer to string descriptor which is updated in the function. The pointed data should persist
	 *  					for application scope. Set the pointer to NULL if you want stack to handle the request.
	 *  \return Returns \ref ErrorCode_t type to indicate success or error condition.
	 *          \retval LPC_OK On success
	 *          \retval ERR_USBD_INVALID_REQ  If application thinks the request is invalid.
	 */
...
struct _USB_CORE_CTRL_T {
	...
	uint8_t  device_speed;/* actually USB speed running on the bus after CHIRP. */
	...
	uint32_t ep_halt;
	uint32_t ep_stall;
	/* Current selected alt intf number. Upper 4 bits store number of alt interface present for given interface */
	...
	ErrorCode_t (*USB_ReqGetStringDesc)(USBD_HANDLE_T hUsb, USB_SETUP_PACKET* setupPacket, uint8_t** pData);
	uint8_t *bos_descriptor;

	const struct USBD_HW_API *hw_api;
};
...
extern ErrorCode_t mwUSB_GetEpHandler(USBD_HANDLE_T hUsb, uint32_t ep_index, USB_EP_HANDLER_T *ep_handler,
									  void * *data);
