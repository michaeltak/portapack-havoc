...
typedef struct USBD_DFU_INIT_PARAM {
	 ...
	 *
	 *  This function will be called when the stack needs to know the status of the current
	 *  transfer. the @a last param when 1 it indicates that the transfer on going is the last
	 *  download packet.
	 *
	 *  <br>
	 *  \note
	 *   This function will not be called when <b>USB_REQ_DFU_GETSTATE</b> requests from Host.
	 *  \param[out] timeout	: If non zero then DFU device will send dfuDNBUSY status when @a last = 0
	 *   or will send dfuMANIFEST when last = 1.
	 *  \param[in]	last	: When 1 indicates that the download is complete, 0 otherwise.
	 *  \return Should return the 'bstatus' value to be sent to host as part DFU_GETSTATUS request.
	 *		\retval DFU_STATUS_OK on Success.
	 *      \retval DFU_STATUS_errXXXXXX on DFU standard error (example: #DFU_STATUS_errTARGET)
	 *      \retval DFU_STATUS_errVENDOR on Vendor specific error (lower 8 bits Bit0-7),
	 * set the upper 8 bits (bit15:bit8) with corresponding index to descriptive string supported by the application.
	 *
	 */
...
typedef struct _USBD_DFU_CTRL_T {
	...
	void (*DFU_Detach)(USBD_HANDLE_T hUsb);
	/* Callback called after USB_REQ_DFU_GETSTATUS */
	uint32_t (*DFU_GetStatus)(uint32_t *timeout, int32_t last);
